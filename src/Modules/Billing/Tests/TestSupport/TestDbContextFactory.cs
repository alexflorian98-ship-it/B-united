...
internal static class TestDbContextFactory
{
    ...
    /// <summary><see cref="Create"/> hands back a single ADO.NET connection shared by every
    /// caller, which serializes all database access through one connection object and cannot
    /// reproduce a genuine concurrent-write race (P3.23.b). This instead opens two independent
    /// connections to the same named, shared-cache in-memory database, so two
    /// <see cref="TestDbContext"/> instances on two different threads can race real, concurrent
    /// <c>SaveChanges</c> calls against the same schema and data — SQLite's own engine-level
    /// locking (with a busy-timeout so a losing writer waits and retries instead of failing with
    /// "database is locked") then serializes the two commits exactly the way Postgres would,
    /// letting the second writer hit the real unique-index violation. Caller must dispose both
    /// connections.</summary>
