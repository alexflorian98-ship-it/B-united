...
- [x] P6.17 Closed 2026-08-18: auto-provision a program's chat room instead of requiring a separate manual admin step — real bug found live-testing: a client who owned "Trai constient" saw an empty, unexplained room list, because no room had ever been created for that program (P3.43.a's own design explicitly allowed "zero or more predefined rooms" per program, created only through the admin-only `CreateChatRoomHandler`). Product decision: every published program now gets exactly one room, named after the program itself, with zero admin action required. New cross-module contract `Chat.Contracts.IProgramChatRoomProvisioner` (mirrors the existing reverse direction — `Chat.Application` already consumes `Content.Contracts.IProgramLookup`), implemented by `ProgramChatRoomProvisioner` (idempotent: a program that already has a room, active or not, is left untouched — an admin's deliberate deactivation is never silently undone), called from `ProgramStatusHandler.PublishAsync` (Content) with the program's default-language title. `ProgramChatRoomBackfillSeeder` runs the same provisioner at startup for every already-published program, covering programs published before this wiring existed, in every environment (not demo-only). Access to the room was already automatic — `ListRoomsHandler.HasAccess` is computed dynamically from real program entitlement, the same as every other program-gated resource — this only guarantees the room itself exists. Live-verified: rebuilt the API container, confirmed `GET /chat/rooms` for the demo client returns one active room named "Trai constient" with `hasAccess: true`, and posting a message to it succeeds. Covered by `ProgramChatRoomProvisionerTests` (Chat.Tests, 3 tests: creates-named-room, no-duplicate-on-second-call, never-resurrects-a-deactivated-room) and a `ContentFlowTests` assertion that publishing invokes the contract with the program's own title.
...
- [~] P7.18 Demo database reset and seed strategy — **richer seed data landed this pass; the dedicated reset command is still the interim `docker compose down -v` procedure**
  - [ ] P7.18.a Not built: no dedicated `reset-demo` command exists yet. Today's only reset path is `docker compose down -v && docker compose up --build`, which drops the whole `postgres-data` Docker volume and re-runs migrations + the idempotent startup seeders — safe (it only ever targets the Compose-managed disposable Postgres container, never a `.env`-configured native install) but coarse-grained, and documented as the interim procedure in `README.md`. A finer-grained, explicitly-Demo/Development-gated in-process reset endpoint is left as a follow-up — deliberately not rushed in this pass given the destructive-operation care such an endpoint needs.
  - [x] P7.18.b `DemoAccountSeeder` (idempotent, Production-gated exactly like P3.32's demo-adapter check — silently skips rather than throwing, since seeded demo *data* is optional and must never block a real Production boot) creates `demo.client@bunited.local`/`demo.expert@bunited.local` (documented password in README, hashed via the real `IPasswordHasher`, not a raw insert), a `Succeeded` purchase + `Payment` + `Invoice` + active `ProgramEntitlement` for the client against the P2.30 demo program, and `ContentProgress` showing one item in-progress (video, 45% watched) and one completed (rich text). Live-verified: logged in as both seeded accounts over real HTTP, confirmed `my-purchases`/`my-entitlements` and `ownershipState: "Owned"` on the program detail read.
