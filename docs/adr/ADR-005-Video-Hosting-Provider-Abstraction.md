...
- **Accepted, documented gap: no real access enforcement on the video URL once issued.** The Api still requires authentication and the (currently stubbed, see P2.09) `IAccessContext` check before it will *hand back* a playback URL/embed reference at all — a logged-out or non-subscribed caller gets nothing from the Api. But once a subscriber has the URL, it is a normal `youtube.com/embed/{id}` reference: not short-lived, not cryptographically tied to that specific viewer or their subscription state, and functions for anyone who obtains it (subscribed or not, even after the original viewer's subscription lapses). This is a real trade-off against the letter of docs/PROMPT.md §18–22's "signed/short-lived playback URL" requirement, made deliberately for V1 development speed and zero cost/credential dependency — not something to treat as already solved.
- **Revisit before a paying public launch.** Once real subscription revenue is at stake, re-evaluate Mux/Cloudflare Stream (both support the originally-specified signed/short-lived/DRM-capable playback model) before relying on YouTube's access characteristics in production with paying subscribers.
